...
for filename in mol_files:
    ...
    print(f" → Average RMSD: {sum(rmsds) / len(rmsds) if rmsds else 0:.4f}")
    ...
    avg_rmsds.append(sum(rmsds) / len(rmsds) if rmsds else 0)
...
if total > 0:
    ...
    print(f"Average RMSD: {sum(avg_rmsds) / len(avg_rmsds):.4f}")
