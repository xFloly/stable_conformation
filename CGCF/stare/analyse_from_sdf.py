...
# ----- Process Random SDF Files -----
mol_files = [f for f in os.listdir(mol_dir) if f.endswith('.sdf')]
random.shuffle(mol_files)
mol_files = mol_files[:num_samples]

for fname in mol_files:
    path = os.path.join(mol_dir, fname)
    supplier = Chem.SDMolSupplier(path, removeHs=False)
    mols = [mol for mol in supplier if mol is not None]

    if not mols:
        print(f"Could not parse {fname} or no valid molecules found.")
        continue

    print(f"\n{fname} — {len(mols)} conformations")
    for conf_id, mol in enumerate(mols):
        try:
            ...
            print(f"Error in {fname} conf {conf_id}: {e}")
